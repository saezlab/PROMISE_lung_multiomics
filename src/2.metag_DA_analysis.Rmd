...
for (rowname in row.names(featTable)) {
  ...
  for (i in seq_along(timepoints)) {
    ...
    # Create matrix
      ...
      df.cal <- add_row(df.cal,  # Fix: Use df.cal instead of p.cal
                       ID = rowname,
                       pval = min(p_values),
                       aucs.mat = aucs.mat, 
                       fc = fc,
...
ggplot(aes(x = fc, y = log10.p)) +
  ...
  #geom_text_repel(data = subset(df.cal, pval < p_cutoff), aes(label = ID), size = 3) +
  # add axis labels 
...
sp.list <- c(
"Turicibacter sanguinis [ref_mOTU_v3_04703]",
"Massilioclostridium coli [ref_mOTU_v3_10237]",
"Massiliomicrobiota species incertae sedis [meta_mOTU_v3_13965]",
"Clostridiales Family XIII. Incertae Sedis species incertae sedis [ext_mOTU_v3_26966]",
"Butyricicoccus pullicaecorum [ref_mOTU_v3_02158]",
"Anaerotruncus colihominis [ref_mOTU_v3_03438]"
)
sp.list <- df.cal %>%
  filter(pval <0.001) %>%
...
p.heat <- ggplot(frame.m, aes(x = sp, y = ID, fill = log2(value))) +
  geom_tile() +
  scale_color_brewer(palette = "BuPu") +
  theme_minimal() +
  labs(title = "Heatmap of Species by Timepoint",
       x = "Species",
       y = "Individuals") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),  # Hide x-axis text
...
meta <- meta %>%
  ...
  mutate(DNA_con_factor=as.factor(
    cut(meta$`DNA concentartion [ng/µl]`, 
        breaks = quantile(meta$`DNA concentartion [ng/µl]`,na.rm = TRUE), 
        labels=c(1,2,3,4)))) %>%
  # library size
  mutate(lib_size_factor=as.factor(
    cut(meta$`ReadCount (Microbiome)`, 
        breaks = quantile(meta$`ReadCount (Microbiome)`,na.rm = TRUE),
        labels=c(1,2,3,4))))
```
